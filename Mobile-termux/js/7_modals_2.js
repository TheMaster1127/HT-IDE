...
function openHtvmToHtvmModal() {
    ...
    const handleSourceSelection = (sourceId) => {
        ...
        fileInput.onchange = async (e) => {
            ...
            overlay.style.display = 'none';
            getActiveTerminalSession()?.xterm.writeln(`\x1b[36mStarting HTVM to HTVM conversion for ${files.length} file(s)...\x1b[0m`);
            ...
            for (const file of Array.from(files)) {
                try {
                    ...
                    getActiveTerminalSession()?.xterm.writeln(`\x1b[32mConverted ${originalFilename} -> ${finalFilename}\x1b[0m`);
                } catch (error) {
                    getActiveTerminalSession()?.xterm.writeln(`\x1b[31mError processing file ${file.name}: ${error.message}\x1b[0m`);
                }
            }
